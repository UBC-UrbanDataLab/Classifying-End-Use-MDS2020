...
def split_datetime(df):
    """Function to create and populate columns for the date, month, and hour value from the timestamp
    of each observation
    ...
    Returns:
        df (pandas.DataFrame): input dataframe with 'date', 'month', 'hour' columns appended
...
def agg_numeric_by_col(df, col_idx, how='all'):
    ...
    Returns:
        df_agg (pandas.DataFrame): Dataframe with col_idx columns as the indexesthe appropriate aggregation(s) 
        as columns, and the count of observations as a column \n
        OR \n
        (None): If invalid how argument passed in
    """
    df = df.copy()
...
def agg_bool_by_col(df, col_idx, how='all'):
    """Function to aggregate the count of boolean value changes the in user specified column(s) 
    using specified aggregation function(s).
    ...
    Args:
        df (pandas.DataFrame): dataframe containing at least a 'value' column \n
        col_idx (list): one or more column indicies to group by when aggregating
        (The "hour" column must always be included for aggregation to work properly)  \n
        how (str or list): 'mean', 'std', 'max', 'min', 'all', (or any function that can
        be passed into the .agg function)
        
    Returns:
        return_df (pandas.DataFrame): Dataframe with col_idx columns as the indexesthe appropriate aggregation(s)
        as columns, and the count of observations as a column
    """
    df = df.copy()
    ...
    is_first = True
    group_list = df.loc[:,'groups'].unique()
...
def agg_cat_by_col(df, col_idx, how='all'):
    ...
    Args:
        ...
        col_idx (list): one or more column indicies to group by when aggregating 
        (The "hour" column must always be included for aggregation to work properly) \n
        how (str or list): 'mean', 'std', 'max', 'min', 'all', (or any function that \n
        can be passed into the .agg function)
        
    Returns:
        return_df (pandas.DataFrame): Dataframe with col_idx columns as the indexes, the appropriate aggregation(s) 
        as columns, and the count of observations as a column
    """
    df = df.copy()
    ...
    cat_2_idx = dict(map(reversed,pd.DataFrame(df.loc[:,'value'].unique()).to_dict()[0].items()))
    df.loc[:,'value'] = df.loc[:,'value'].apply(lambda x: cat_2_idx[x])
    # Converting indexes to group by to the column names
    group_names = df.columns[col_idx].values.tolist()
    group_names.extend(['hour']) # First aggregation is by hour
    # Sorting observations by their timestamp
...
def combine_mixed_agg(df, struct_df, col_idx):
    """Function to aggregate any duplicate observations within a dataframe, this function is 
    required in the case that there are any mixed datatypes between aggregation groups 
    (ex. if an aggregation group has continuous and categorical data these would be stored 
    two aggregations and need to be aggregated further down to one observation)
    ...
    Returns:
        df (pandas.DataFrame): Dataframe with any duplicate observations aggregated down to one observation
    """
    df = df.copy()
...
def agg_all(df, col_idx, how='all', last_idx_to_col=True):
    ...
    Args:
        df (pandas.DataFrame): dataframe containing at least a 'value' column \n
        col_idx (list): one or more column indicies to group by when aggregating  \n
        how (str or list): 'mean', 'std', 'max', 'min', 'all', (or any function that can be 
        passed into the .agg function) \n
        last_idx_to_col (bool): A boolean value indicating if the last index in the col_idx 
        list should be made into columns in the output (True means yes and is the default)
        
    Returns:
        pandas.DataFrame: Dataframe with col_idx columns (minus the last index in the list if 
        last_idx_to_col=True) as the indexes, the appropriate aggregation(s) (grouped by the unique
        values of the column of the last value in col_idx if last_idx_to_col=True) as columns, 
...
def append_agg(df1, df2, struct_df, col_idx, last_idx_to_col=True):
    ...
    Args:
        ...
        struct_df (pandas.DataFrame): the original dataframe used to generate df1 or df2 
        (just needs the structure so could just pass the head) \n
        col_idx (list): the column indicies to group by when aggregating \n
        how (str or list): 'mean', 'std', 'max', 'min', 'all', (or any function that can
        be passed into the .agg function) \n
        last_idx_to_col (bool): A boolean value indicating if the last index in the 
        col_idx list should be made into columns in the output (True means yes and is the default)
        ...
        if col=='count':
            ...
            try:
                ...
                if col+"_1" in temp_df.columns.tolist():
                    ...
                    temp_df.loc[:,col+'_1'] = 0
                    temp_df.loc[:,col+'_2'] = 0
    drop_list = [col+"_1" for col in cols]
    drop_list.extend([col+"_2" for col in cols])
