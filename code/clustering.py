...
### ~ Clustering Function ~ ###
def cluster(df, clust_type, num_clusts = None, continuous_columns = None, input_type='original', linkage_type = 'single'):
    ...
    Args:
        ...
        \t OR (np.array): numpy array of Gower's distances between observations (if input_type='gowers') \n
        \t OR (pandas.DataFrame): dataframe of multidimensionaly scaled values (if input_type='mds') \n
        ...
        linkage_type (str): user defined linkage type, only required for agglomerative clustering, 
        options are 'ward', 'complete', 'average', 'single' (default='single')
   ...
   # (this process allows for clustering mixed continuous and categorical within the same predictor space)     
    if clust_type in needs_mds:
        fit_data = multidim_scale(fit_data, num_dim=2)
    ...
    if clust_type == 'kmeans':
        ...
        # Needs df, clust_type, num_clusts, continuous_columns
        if input_type != 'mds':
            model = AgglomerativeClustering(affinity='precomputed', linkage = linkage_type, n_clusters=num_clusts).fit(fit_data)
        else:
            model = AgglomerativeClustering(linkage = linkage_type, n_clusters=num_clusts).fit(fit_data)
        preds = model.labels_
    elif clust_type == 'dbscan':
        # Needs df, clust_type, continuous_columns
        if input_type != 'gowers':
            print("DBSCAN doesn't accept mds or original input type, please provide input as a Gower's distance matrix.")
        else:
        ...
        # Needs df, clust_type, continuous_columns
        if input_type != 'gowers':
            print("HDBSCAN doesn't accept mds or original input type, please provide input as a Gower's distance matrix.")
        else:
